...
import { ArrowUpDown, Eye, Edit, MapPin } from "lucide-react";
import Link from "next/link";

interface NationalPermitTableProps {
  vehicles: Vehicle[];
...
export function NationalPermitTable({
  vehicles,
  onSort,
  sortKey,
  sortDirection,
  ...
  return (
    <div className="bg-white border border-[#E5E5E5] rounded-[16px] shadow-sm overflow-hidden flex flex-col">
        ...
        <table className="min-w-full divide-y divide-[#E5E5E5]">
          <thead className="bg-[#F8FAFC]">
            <tr>
              ...
              <SortHeader label="Authorized States" sortKeyName="state_info" />
              <SortHeader label="City" sortKeyName="city" />
              <th className="px-4 py-3 text-left text-[12px] font-semibold text-[#777777] uppercase tracking-wider">
                Postal Address
              </th>
              <SortHeader label="NP Valid Upto" sortKeyName="expiry_date" />
          ...
          <tbody className="bg-white divide-y divide-[#E5E5E5]">
            {isLoading ? (
              ...
              <tr>
                ...
                >
                  No vehicles matching the selected national permit criteria.
                </td>
              </tr>
            ) : (
              vehicles.map((v) => (
                ...
                >
                  ...
                  <td className="px-4 py-3.5 text-[13px] text-[#333333]">
                    {v.national_permit?.state_info ? (
                      <span className="inline-flex items-center gap-1 font-medium text-slate-800 bg-slate-100 px-2 py-0.5 rounded border border-slate-200 text-[12px]">
                  ...
                  <td className="px-4 py-3.5 text-[13px] text-[#555555] max-w-[200px] truncate">
                    {v.national_permit?.address || "—"}
                  </td>
                  <td className="px-4 py-3.5 text-[13px] font-medium text-[#111111]">
                    {formatDate(v.national_permit?.expiry_date)}
                  </td>
                  <td className="px-4 py-3.5">{getStatusBadge(v.national_permit_status)}</td>
                  <td
      ...
      <div className="px-4 py-3 border-t border-[#E5E5E5] bg-[#F8FAFC] flex justify-between items-center text-[13px] text-[#777777]">
        <span>
          Showing <strong>{vehicles.length}</strong> national permit records
        </span>
